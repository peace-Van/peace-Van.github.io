...
So what does the theoretical temperature tell us? As far as I know it provides a hint of the scaling of the parameter $T$ on the given dataset. So we can better estimate an initial value for trial by knowing its order of magnitude - whether we should start the trial from some value below ten, or some tens, or some hundreds. It's also plausible to rescale the input data for a comfortable scale of $T$. If you feel $T$ is too small, you may squeeze the data space (divide the data by some constant larger than 1), and vice versa.    
