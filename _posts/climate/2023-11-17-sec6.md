...
Clustering result on $T=5.2$. Cluster names are going to be explained in Sec 7. Right-click for full image.   
...
Clustering result on $T=1.6$. This is the maximum temperature to have no singleton cluster. The result is similar to that of a high-level Ward hierarchical clustering. Right-click for full image.   
...
> It's worth noting that compared with the result of $T=5.2$, the algorithm gives one more cluster on $T=5.0$. This situation is not common. Analogously, some substances have lower solubility on higher temperature, like [calcium hydroxide in water](https://en.wikipedia.org/wiki/Calcium_hydroxide#Retrograde_solubility). Also the cluster morphology is largely different in the two results.   
